...
import {
  KIDS_GIVE_FAILED,
  KIDS_GIVE_SUBMIT,
  KIDS_GIVE_SUBMITTING,
  kidsGiveAvailableHint,
  kidsGiveOverdraftMessage,
  kidsGiveSheetIntro,
  kidsGiveSheetTitle,
} from '@/lib/brand'
...
export default function FundKidSheet({
  kidId,
  kidName,
  available,
  open,
  ...
  const availableHint =
    available >= 0 && !overdraft
      ? kidsGiveAvailableHint(availableLabel)
      : null
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      setSubmitError(
        err instanceof Error ? err.message : KIDS_GIVE_FAILED,
      )
    } finally {
      setSubmitting(false)
    }
  ...
  return (
    <Sheet open onClose={handleClose} aria-label={kidsGiveSheetTitle(kidName)}>
      <form onSubmit={(e) => void onSubmit(e)} className="space-y-4">
        <header className="flex items-baseline justify-between">
          <h2 className="text-lg font-semibold text-zinc-300">
            {kidsGiveSheetTitle(kidName)}
          </h2>
          <button
            type="button"
            onClick={handleClose}
        ...
        <p className="text-sm text-zinc-400">
          {kidsGiveSheetIntro(availableLabel)}
        </p>
        ...
        >
          {submitting ? KIDS_GIVE_SUBMITTING : KIDS_GIVE_SUBMIT}
        </button>
      </form>
    </Sheet>
  )
