-- =====================================================================
-- Home breakdown: expose when the bank balance was last synced.
--
-- The Unallocated card shows a subtle "Updated Xm ago" line for every
-- role. Children can't read the accounts table (RLS), so we surface a
-- single family-wide timestamp from this SECURITY DEFINER function.
--
-- Privacy: a bare timestamp leaks no balances, account names, or
-- amounts — only when an adult last refreshed the family bank. The
-- child role's isolation (no balances, no other members' data) is
-- unaffected. See CONTEXT.md § Security.
-- =====================================================================
...
begin
  ...
  -- Family-wide latest cash sync. Same value for every role (definer);
  -- children get the timestamp without the underlying account rows.
