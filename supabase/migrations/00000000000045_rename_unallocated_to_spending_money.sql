-- Align ledger naming with product language: "spending money" (UI) = this pool in SQL.
-- NULL bucket id in move_money still means the spending-money pool.

alter table public.transactions
  rename column unallocated_balance_before to spending_money_balance_before;

alter table public.transactions
  rename column unallocated_balance_after to spending_money_balance_after;

-- Same formula as member_available_balance (migration 16); renamed for ubiquitous language.
create or replace function public.member_spending_money(p_member_id uuid)
returns numeric
language plpgsql
stable
security definer
...
revoke all on function public.member_spending_money(uuid) from public, anon, authenticated;
grant execute on function public.member_spending_money(uuid) to service_role;

-- Same Supabase default-grant gap as member_spending_money (migration 16).
...
create or replace function public.get_spending_money_balance()
returns numeric
language sql
stable
security definer
set search_path = public
as $$
  select public.member_spending_money(public.auth_member_id());
$$;

revoke all on function public.get_spending_money_balance() from public;
grant execute on function public.get_spending_money_balance() to authenticated;
grant execute on function public.get_spending_money_balance() to service_role;
...
begin
  ...
  v_spending_money := public.member_spending_money(v_member_id);
  ...
  return jsonb_build_object(
    'spending_money', v_spending_money,
    'total_cash', v_cash,
...
declare
  ...
  v_from_name        text;
  v_to_name          text;
  v_spending_money_before numeric;
  v_spending_money_after numeric;
  v_spending_money   numeric;
...
begin
  ...
  if p_from_bucket_id is null or p_to_bucket_id is null then
    v_spending_money_before := public.member_spending_money(v_caller_member_id);
  end if;
  ...
  if p_from_bucket_id is not null then
    ...
    v_spending_money := public.member_spending_money(v_caller_member_id);
    if v_spending_money < p_amount then
      raise exception 'insufficient spending money balance' using errcode = '23514';
    end if;
  end if;
  ...
  if p_from_bucket_id is null or p_to_bucket_id is null then
    v_spending_money_after := public.member_spending_money(v_caller_member_id);
  end if;
  ...
  insert into public.transactions (
    ...
    spending_money_balance_before,
    spending_money_balance_after,
    from_member_id,
    note
  ) values (
    v_caller_family,
    ...
    v_to_balance_before,
    v_to_balance_after,
    v_spending_money_before,
    v_spending_money_after,
    v_caller_member_id,
    p_note
  )
...
declare
  ...
  v_to_name          text;
  v_available        numeric;
  v_spending_money_member_id uuid;
  ...
  v_spending_money_balance_before numeric;
  v_spending_money_balance_after numeric;
  v_transaction_id   uuid;
begin
  if v_caller_member_id is null or v_caller_family is null then
    raise exception 'not authenticated' using errcode = '28000';
  ...
  if v_caller_role in ('admin', 'member') and v_to_role = 'child' then
    v_available := public.member_spending_money(v_caller_member_id);
    if coalesce(v_available, 0) < p_amount then
      raise exception 'insufficient spending money balance' using errcode = '23514';
    end if;
    v_spending_money_member_id := v_caller_member_id;
    v_spending_money_balance_before := v_available;
    ...
    if v_to_role = 'child' then
      ...
      v_spending_money_member_id := p_to_member_id;
      v_spending_money_balance_before := public.member_spending_money(p_to_member_id);
    end if;
  end if;
  ...
  insert into public.transactions (
    ...
    v_spending_money_balance_before,
    null,
    p_note
  )
  returning id into v_transaction_id;

  if v_spending_money_member_id is not null then
    v_spending_money_balance_after :=
      public.member_spending_money(v_spending_money_member_id);
    update public.transactions
       set spending_money_balance_after = v_spending_money_balance_after
...
drop function if exists public.member_available_balance(uuid);
