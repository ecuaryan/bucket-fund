-- =====================================================================
-- "Refreshed X ago" means a bank pull, not a manual edit.
--
-- Manual-account RPCs stamp last_synced_at on every add/update (useful
-- in Admin as "when did I last set this"), but the home breakdown's
-- family-wide bank_last_synced_at fed the Buckets hero's refresh label —
-- so adding manual cash announced "Refreshed just now" on a control
-- whose meaning is "re-pull from Teller". Scope the timestamp to
-- source = 'teller' rows. Manual-only families now report null (they
-- have no refresh control either). Everything else is identical to
-- migration 79's definition.
-- =====================================================================
