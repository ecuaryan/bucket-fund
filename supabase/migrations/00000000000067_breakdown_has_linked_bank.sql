-- =====================================================================
-- Home breakdown: expose whether the signed-in member personally owns a
-- Teller-linked bank account.
--
-- Why: the Buckets refresh control for a child was gated on the
-- family-wide `bank_last_synced_at` (non-null whenever ANY family bank
-- has ever synced). That made virtual kids — who own no accounts — show
-- a refresh button as soon as a parent linked a bank, with nothing of
-- their own to refresh. `bank_cash` is child-scoped but is 0 for a
-- linked kid who hasn't synced yet (or sits at $0), so it can't gate the
-- button on its own either.
--
-- `has_linked_bank` is the authoritative, balance- and sync-independent
-- signal: true iff the caller owns a `source = 'teller'` account. Same
-- predicate `give_money` / `return_from_child` already use via
-- `member_has_linked_account`. Privacy is unchanged — a bare boolean
-- leaks no balances or account details.
-- =====================================================================
