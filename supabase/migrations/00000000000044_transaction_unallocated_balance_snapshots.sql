-- Snapshot unallocated (member_available_balance for the affected pool) so History
-- can show both sides of bucket moves and sends.

alter table public.transactions
  add column unallocated_balance_before numeric(14, 2),
  add column unallocated_balance_after numeric(14, 2);
...
declare
  ...
  v_from_name        text;
  v_to_name          text;
  v_unallocated_before numeric;
  v_unallocated_after numeric;
...
begin
  ...
  if p_from_bucket_id is null or p_to_bucket_id is null then
    v_unallocated_before := public.member_available_balance(v_caller_member_id);
  end if;
  ...
  if p_from_bucket_id is null or p_to_bucket_id is null then
    v_unallocated_after := public.member_available_balance(v_caller_member_id);
  end if;
...
declare
  ...
  v_unallocated_balance_before numeric;
  v_unallocated_balance_after numeric;
  v_transaction_id   uuid;
begin
  if v_caller_member_id is null or v_caller_family is null then
    raise exception 'not authenticated' using errcode = '28000';
  ...
  if v_caller_role in ('admin', 'member') and v_to_role = 'child' then
    ...
    v_unallocated_member_id := v_caller_member_id;
    v_unallocated_balance_before := v_available;
    ...
    if v_to_role = 'child' then
      ...
      v_unallocated_member_id := p_to_member_id;
      v_unallocated_balance_before := public.member_available_balance(p_to_member_id);
    end if;
  end if;
  ...
  insert into public.transactions (
    ...
    v_unallocated_balance_before,
    null,
    p_note
  )
  returning id into v_transaction_id;

  if v_unallocated_member_id is not null then
    v_unallocated_balance_after :=
      public.member_available_balance(v_unallocated_member_id);
    update public.transactions
       set unallocated_balance_after = v_unallocated_balance_after
